...
const Entidades = () => {
  ...
  useEffect(() => {
    applyFilters();
  }, [searchField, searchTerm, additionalFilters, entidades]);
  ...
  const exportToExcel = () => {
    console.log("Exportando a Excel", filteredEntidades);
    setIsExportDropdownOpen(false);
  };

  const exportToPdf = () => {
    console.log("Exportando a PDF", filteredEntidades);
    setIsExportDropdownOpen(false);
  };

  const exportToCsv = () => {
    console.log("Exportando a CSV", filteredEntidades);
    setIsExportDropdownOpen(false);
  };
  ...
  const currentRows = filteredEntidades.slice(indexOfFirstRow, indexOfLastRow);
  const totalPages = Math.ceil(filteredEntidades.length / rowsPerPage);
      ...
      <>
        ...
        <div className={styles.containerentidades} style={{ marginLeft: isMenuExpanded ? "100px" : "20px" }}>
          ...
          <div className={styles.topControls}>
            ...
            >
              <FaPlus /> {showForm ? 'Ver Entidades' : 'Agregar Entidad'}
            </button>
          </div>
          ...
          {showForm ? (
            <div className={styles.containerUsuarios}>
              <h2 className={styles.titulo}>
                {editingId ? 'Editar Entidad' : 'Formulario de Creación de Entidad'}
              </h2>
            ...
            <>
              ...
              <div className={styles.usersTableContainer}>
                <h2 className={styles.sectionTitle}>Entidades Registradas ({filteredEntidades.length})</h2>
                          ...
                          <tr key={entidad.id_entidad}>
                            ...
                            <td>
                              ...
                              <button
                                ...
                                onClick={() => handleDelete(entidad.id_entidad)}
                              >
                                Eliminar
                              </button>
                            </td>
              ...
              <div className={styles.paginationControls}>
                <div className={styles.rowsPerPageSelector}>
                  ...
                  <span className={styles.rowsInfo}>
                    Mostrando {indexOfFirstRow + 1}-{Math.min(indexOfLastRow, filteredEntidades.length)} de {filteredEntidades.length} registros
                  </span>
                </div>
